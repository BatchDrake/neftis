...
rbtree_new (void)
{
  ...
  if ((new = salloc (sizeof (rbtree_t))) == NULL)
    return NULL;

  memset (new, 0, sizeof (rbtree_t));
...
void
rbtree_debug_node (struct rbtree_node *node)
{
  debug ("  node_%p [label = \"%s\"];\n", node, (char *) node->data);

  if (node->left)
  {
    debug ("  node_%p -> node_%p [color=green];\n", node, node->left);
    rbtree_debug_node (node->left);
  }
  
  if (node->right)
  {
    debug ("  node_%p -> node_%p [color=red];\n", node, node->right);
    rbtree_debug_node (node->right);
  }
}

void
rbtree_debug (rbtree_t *tree)
{
  debug ("digraph G\n{\n");

  if (tree->root != NULL)
    rbtree_debug_node (tree->root);
...
rbtree_node_new (rbtree_t *tree, int64_t key, void *data)
{
  ...
  if ((new = salloc (sizeof (struct rbtree_node))) == NULL)
    return NULL;

  memset (new, 0, sizeof (struct rbtree_node));
...
rbtree_search (rbtree_t *tree, int64_t key, enum rbtree_node_search_mode mode)
{
  ...
  if (key < closest->key && mode == RB_LEFTWARDS)
    closest = closest->prev;
  else if (closest->key < key && mode == RB_RIGHTWARDS)
